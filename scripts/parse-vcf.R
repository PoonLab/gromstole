...
index <- match(longshot$label, ont$label)

longshot$illumina <- ont$frequency[index]

png(file='temp.png', width=5*150, height=5*150, res=150)
par(mar=c(5,5,1,1))
plot(longshot$rel.freq, longshot$illumina, xlim=c(0,1), ylim=c(0,1), type='n')
text(longshot$rel.freq, longshot$illumina, label=longshot$label, cex=0.5)
abline(a=0, b=1)
dev.off()

tab1 <- read.csv('data/landgraff-table1.csv', header=T)
index <- match(tab1$label, longshot$label)
...
foo <- ont[ont$coverage > 100, ]
plot(foo$frequency, foo$nanopore)
