...
\end{tikzpicture}
\caption{The structure of our pipeline. Wastewater samples are processed to find the number of times each mutation is observed along with the coverage at every position (relative to the reference genome). This information is combined with a list of mutations believed to be unique to the variant of interest (possibly from counts of mutations in known lineages from NextStrain). It is important to have the coverage for the position of each ``unique'' mutation, regardless of whether this was observed in the wastewater samples. The proportion is found via a binomial regression model.}
\end{figure}
